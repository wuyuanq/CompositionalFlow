...
program infile_RSTi_compositionalTwoPhaseFlow
    ...
    integer :: i
    real(kind=8) :: PCONST = 2.D6

    modelCase%Nc = 2
    modelCase%Temp = 260.0
    modelCase%Lx = 4
    modelCase%Ly = 4
    modelCase%timeEnd = 0.2*365*24*3600.0
    modelCase%nx = 40
    modelCase%ny = 40
    modelCase%nt = 0.2*365*24*10
    ...
    modelCase%PwBdryX(2,1) = PCONST
    allocate(modelCase%PwBdryY(modelCase%nx,2))
    modelCase%PwBdryY(:,:) = 0.0
    modelCase%PwBdryY(modelCase%nx,1) = PCONST
