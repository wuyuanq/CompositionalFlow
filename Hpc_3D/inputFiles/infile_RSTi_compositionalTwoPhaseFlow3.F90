...
program infile_RSTi_compositionalTwoPhaseFlow
    ...
    modelCase%nx = 10*times!!!!
    modelCase%ny = 20*times!!!!
    modelCase%nz = 40*times!!!!
    modelCase%nt = 0.1*365*24*8!!!!!

    call proceAlloc(8, modelCase%nx, modelCase%ny, modelCase%nz, modelCase%pncols, modelCase%pnrows, modelCase%pnlays)
    ...
    modelCase%src(:,:,:,:) = 0.0
    modelCase%src(1,1:times,1:times,1:times) = 0.2

    allocate(modelCase%isDiriX(2,modelCase%ny,modelCase%nz))
    modelCase%isDiriX(:,:,:) = 0
    modelCase%isDiriX(2,modelCase%ny-times+1:modelCase%ny,modelCase%nz-times+1:modelCase%nz) = 1!!!!!
    allocate(modelCase%isDiriY(modelCase%nx,2,modelCase%nz))
    modelCase%isDiriY(:,:,:) = 0
    modelCase%isDiriY(modelCase%nx-times+1:modelCase%nx,2,modelCase%nz-times+1:modelCase%nz) = 1!!!!!
    allocate(modelCase%isDiriZ(modelCase%nx,modelCase%ny,2))
    modelCase%isDiriZ(:,:,:) = 0
    modelCase%isDiriZ(modelCase%nx-times+1:modelCase%nx,modelCase%ny-times+1:modelCase%ny,2) = 1!!!!!

    allocate(modelCase%PwBdryX(2,modelCase%ny,modelCase%nz))
    modelCase%PwBdryX(:,:,:) = 0.0
    modelCase%PwBdryX(2,modelCase%ny-times+1:modelCase%ny,modelCase%nz-times+1:modelCase%nz) = PCONST
    allocate(modelCase%PwBdryY(modelCase%nx,2,modelCase%nz))
    modelCase%PwBdryY(:,:,:) = 0.0
    modelCase%PwBdryY(modelCase%nx-times+1:modelCase%nx,2,modelCase%nz-times+1:modelCase%nz) = PCONST
    allocate(modelCase%PwBdryZ(modelCase%nx,modelCase%ny,2))
    modelCase%PwBdryZ(:,:,:) = 0.0
    modelCase%PwBdryZ(modelCase%nx-times+1:modelCase%nx,modelCase%ny-times+1:modelCase%ny,2) = PCONST
