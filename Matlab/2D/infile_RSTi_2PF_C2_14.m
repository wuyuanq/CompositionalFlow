...
model.zBdryX = zeros(model.Nc, 2, ny);
model.zBdryY = zeros(model.Nc, nx, 2);

model.zInit = zeros(model.Nc,nx,ny);    
model.zInit(1:model.Nc, 1:end, 1:end) = 0.5; 

model.PwBdryX = zeros(2, ny);
model.PwBdryX(2, 1) = PCONST;   % Pa 
model.PwBdryY = zeros(nx, 2);
model.PwBdryY(nx, 1) = PCONST;   % Pa 

model.PwInit = zeros(nx,ny);  
model.PwInit(1:end, 1:end) = PCONST;
...
RST_compositionalTwoPhaseFlow(model)   
