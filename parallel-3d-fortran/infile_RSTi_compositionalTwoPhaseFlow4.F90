...
program infile_RSTi_compositionalTwoPhaseFlow
    ...
    modelCase%nz = 1 !!40*times!!!!
    modelCase%nt = 0.1*365*24/2!!!!!

    call proceAlloc(512, modelCase%nx, modelCase%ny, modelCase%nz, modelCase%pncols, modelCase%pnrows, modelCase%pnlays)
    ...
    modelCase%isDiriX(:,:,:) = 0
    modelCase%isDiriX(2,1:times,1:modelCase%nz) = 1!!!!!
    allocate(modelCase%isDiriY(modelCase%nx,2,modelCase%nz))
    modelCase%isDiriY(:,:,:) = 0
    modelCase%isDiriY(modelCase%nx-times+1:modelCase%nx,1,1:modelCase%nz) = 1!!!!!
    ...
    do j = 1, times
        do k = 1, modelCase%nz
            modelCase%PwBdryX(2,j,k) = PCONST
        end do
    end do
    allocate(modelCase%PwBdryY(modelCase%nx,2,modelCase%nz))
    modelCase%PwBdryY(:,:,:) = 0.0
    modelCase%PwBdryY(modelCase%nx-times+1:modelCase%nx,1,1:modelCase%nz) = PCONST
